...
class Helpers {
	...
	public static function locate_template( $tpl ) {
		...
		$path = apply_filters( 'ARI/Helpers/locate_template/templates', array( 'views/' . ARI_VIEWS_FOLDER_NAME . '/' . $tpl . '.php' ), $tpl, __NAMESPACE__ );
		...
		// Locate on the files
		if ( is_file( ARI_DIR . 'views/' . $tpl . '.php' ) ) {// Use builtin template
			return ( ARI_DIR . 'views/' . $tpl . '.php' );
		}
